...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	switch (Message) {
		...
		/* 关闭窗口时，询问是否关闭此应用 */
		case WM_CLOSE: { //关闭时
			int x = MessageBox(NULL, "是否关闭此应用？", "询问", MB_YESNO | MB_ICONQUESTION);
			if (x == IDYES) {
		...
		case WM_COMMAND: { //命令
			switch (wParam) {
				...
				case 666: { //抖
					GetWindowRect(hwnd, &rect); //left左，right右，top上，bottom下
					MoveWindow(hwnd, rect.left + SHAKE, rect.top, rect.right - rect.left, rect.bottom - rect.top, TRUE); //右
					Sleep(20);
					MoveWindow(hwnd, rect.left - SHAKE, rect.top, rect.right - rect.left, rect.bottom - rect.top, TRUE); //左
					Sleep(20);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, "WindowClass", "快捷菜单 1.2_此应用由XGACY开发", WS_VISIBLE | WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX,
	                      CW_USEDEFAULT, /* x */
	                      CW_USEDEFAULT, /* y */
	                      390, /* 窗口长 */
	                      360, /* 窗口宽 */
