			...
			if (wParam == 2) { // 判断计时器ID
				...
				if (defaultFile.is_open()) {
					...
					while (std::getline(defaultFile, line)) {
						if (isFirstLine) {
							// 获取窗口样式
							LONG style = GetWindowLong(ipComboBox, GWL_STYLE);
							// 判断是否为下拉列表框
							if (style & CBS_DROPDOWN) {
								...
								// 不是下拉列表框
								// 创建下拉列表框
								DestroyWindow(ipComboBox); // 删除共享目录输入框
								ipComboBox = CreateWindow("COMBOBOX", "", WS_VISIBLE | WS_CHILD | WS_BORDER | CBS_DROPDOWN | CBS_AUTOHSCROLL, 335, 117, 150, 200, hwnd, NULL, NULL, NULL); // 加上了CBS_AUTOHSCROLL以便可以无限输入
							}
							isFirstLine = false;
						}
				...
				if (defaultFile2.is_open()) {
					...
					while (std::getline(defaultFile2, line)) {
						if (isFirstLine) {
							// 获取窗口样式
							LONG style = GetWindowLong(sharedFolderEdit, GWL_STYLE);
							// 判断是否为下拉列表框
							if (style & CBS_DROPDOWN) {
								// 是下拉列表框
							} else {
